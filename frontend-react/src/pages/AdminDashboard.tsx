...
const AdminDashboard = () => {
    ...
    const loadSystemData = async () => {
        ...
        try {
            ...
            if (Array.isArray(docsRes.data)) setDoctors(docsRes.data);

            const patsRes = await api.get('/patients');
            if (Array.isArray(patsRes.data)) setPatientsList(patsRes.data);

            try {
                const examsRes = await api.get('/examinations/all');
                if (Array.isArray(examsRes.data)) setExaminations(examsRes.data);
    ...
    const handleDoctorSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            loadSystemData();
        } catch (err: any) { setErrorMsg(err.response?.data?.message || 'Failed to register doctor.'); }
    };
    ...
    const handlePatientSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            loadSystemData();
        } catch (err: any) { setErrorMsg(err.response?.data?.message || 'Failed to register patient.'); }
    };

    // --- ФРОНТЕНД СТАТИСТИКИ (За максимална точност) ---
    ...
    // 1. Изчисляване на пациенти към личен лекар
    doctors.filter(d => d.gp).forEach(doc => {
        // Проверяваме дали пациента има този лекар по ID или Име (в зависимост от бекенда ти)
    ...
    // 2. Изчисляване на приходи и болнични
        ...
        <Container className="py-5" style={{ maxWidth: '1200px' }}>
            ...
            <Tab.Container defaultActiveKey="stats">
                ...
                <Tab.Content>
                                    ...
                                    <Card.Body className="p-0" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                                        <Table hover className="mb-0 small">
                                            ...
                                            {Object.entries(stats.visitsPerDoctor).map(([docName, visits], idx) => (
                                                <tr key={idx}>
                                                    {/* ОПРАВЕНО: Премахнато е двойното изписване "Dr. " */}
                                                    ...
                                                    <td className="text-end pe-4 fw-bold text-success">
                                                        {/* ОПРАВЕНО: Използваме директно docName за търсене в масива с приходи */}
                            ...
                            <Row>
                                    ...
                                    <Card className="border-0 shadow-sm mb-4 border-top border-primary border-3">
                                        ...
                                        <Card.Body className="p-0" style={{ maxHeight: '250px', overflowY: 'auto' }}>
                                            <Table hover responsive className="mb-0">
                                                <thead className="table-light text-muted small"><tr><th>Name</th><th>Specialty</th><th>Role</th></tr></thead>
                                                <tbody>
                                                {doctors.map(d => (
                                                    <tr key={d.id}><td className="fw-bold">Dr. {d.name}</td><td>{d.specialty}</td><td>{d.gp ? <Badge bg="primary">GP</Badge> : <Badge bg="secondary">Specialist</Badge>}</td></tr>
                                                ))}
                                                </tbody>
                                            </Table>
                                        </Card.Body>
                                ...
                                <Col lg={6}>
                                    <Card className="border-0 shadow-sm mb-4 border-top border-success border-3">
                                        ...
                                        <Card.Body className="p-0" style={{ maxHeight: '250px', overflowY: 'auto' }}>
                                            <Table hover responsive className="mb-0">
                                                <thead className="table-light text-muted small"><tr><th>Name</th><th>EGN</th><th>Insurance</th></tr></thead>
                                                <tbody>
                                                {patientsList.map(p => (
                                                    <tr key={p.id}><td className="fw-bold">{p.name}</td><td>{p.egn}</td><td>{p.healthInsured ? <Badge bg="success">Yes</Badge> : <Badge bg="danger">No</Badge>}</td></tr>
                                                ))}
                                                </tbody>
                                            </Table>
                                        </Card.Body>
                                ...
                                {/* ИСТОРИЯ НА ПРЕГЛЕДИТЕ С ФИЛТРИ */}
                                <Col lg={12}>
                    ...
                    {/* --- ТАБ 3: РЕГИСТРАЦИЯ (Без промяна) --- */}
                    <Tab.Pane eventKey="registry">
                        {/* ТУК ОСТАВА СТАРИЯТ КОД ЗА ФОРМИТЕ ЗА РЕГИСТРАЦИЯ, КОЙТО СИ ИМАШ */}
                                    ...
                                    <Tab.Content>
                                        <Tab.Pane eventKey="doctor">
                                            <Form onSubmit={handleDoctorSubmit} className="p-3">
                                                <Row>
                                                    <Col md={6}>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Full Name</Form.Label><Form.Control type="text" value={doctorFormData.name} onChange={e => setDoctorFormData({ ...doctorFormData, name: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">UIN</Form.Label><Form.Control type="text" value={doctorFormData.uin} onChange={e => setDoctorFormData({ ...doctorFormData, uin: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Specialty</Form.Label><Form.Control type="text" value={doctorFormData.specialty} onChange={e => setDoctorFormData({ ...doctorFormData, specialty: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Check type="checkbox" label="Is General Practitioner (GP)?" checked={doctorFormData.isGp} onChange={e => setDoctorFormData({ ...doctorFormData, isGp: e.target.checked })} className="text-dark fw-bold" /></Form.Group>
                                                    </Col>
                                                    <Col md={6}>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Username</Form.Label><Form.Control type="text" value={doctorFormData.username} onChange={e => setDoctorFormData({ ...doctorFormData, username: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Email</Form.Label><Form.Control type="email" value={doctorFormData.email} onChange={e => setDoctorFormData({ ...doctorFormData, email: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Password</Form.Label><Form.Control type="password" value={doctorFormData.password} onChange={e => setDoctorFormData({ ...doctorFormData, password: e.target.value })} required /></Form.Group>
                                                    </Col>
                                                </Row>
                                        ...
                                        <Tab.Pane eventKey="patient">
                                            <Form onSubmit={handlePatientSubmit} className="p-3">
                                                <Row>
                                                    <Col md={6}>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Full Name</Form.Label><Form.Control type="text" value={patientFormData.name} onChange={e => setPatientFormData({ ...patientFormData, name: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">EGN</Form.Label><Form.Control type="text" value={patientFormData.egn} onChange={e => setPatientFormData({ ...patientFormData, egn: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Check type="checkbox" label="Health Insured?" checked={patientFormData.isHealthInsured} onChange={e => setPatientFormData({ ...patientFormData, isHealthInsured: e.target.checked })} className="text-dark fw-bold" /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Personal GP</Form.Label>
                                                    ...
                                                    <Col md={6}>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Username</Form.Label><Form.Control type="text" value={patientFormData.username} onChange={e => setPatientFormData({ ...patientFormData, username: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Email</Form.Label><Form.Control type="email" value={patientFormData.email} onChange={e => setPatientFormData({ ...patientFormData, email: e.target.value })} required /></Form.Group>
                                                        <Form.Group className="mb-3"><Form.Label className="text-muted fw-bold small">Password</Form.Label><Form.Control type="password" value={patientFormData.password} onChange={e => setPatientFormData({ ...patientFormData, password: e.target.value })} required /></Form.Group>
                                                    </Col>
                                                </Row>
